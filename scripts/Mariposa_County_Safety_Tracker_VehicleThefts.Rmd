...
![<popupchatter>Updated `r format(Sys.time(), '%B %d, %Y')` by the ABC30 Data Team</popupchatter>](https://github.com/abcotvdata/safetytracker_fresno/raw/main/docs/banner_placeholder.png)
...
autothefts_county <- countywide_autotheft %>% filter(county=="Mariposa County")
```

<h2>ABC30 is tracking crime and safety in communities across Mariposa County. 
...
There were <h8>`r prettyNum(autothefts_county$total21,big.mark=",")`</h8> vehicle thefts in 2021 across Mariposa County, according to data that law enforcement agencies report to the state of California.
